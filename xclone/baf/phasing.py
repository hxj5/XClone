...
from ..utils.xmath import normalize, loglik_amplify, safe_log
...
def Local_Phasing(AD, DP, min_iter=10, max_iter=1000, epsilon_conv=1e-2,
    ...
    if init_mode == 'warm':    # Option 1: warm initialization (usually good?)
        Z = np.zeros((N, 2))
        Z[:, 0] = (AD.sum(1) / (DP.sum(1) + eps)).reshape(-1)
        Z[:, 1] = 1 - Z[:, 0]
    ...
    thetas = (AD.T @ Z + BD.T @ (1 - Z)) /   \
        (DP.T.sum(axis = 1, keepdims = True) + eps)
    ...
    # likelihood
    _logLik_mat = AD @ safe_log(thetas) + BD @ safe_log(1 - thetas)
    ...
    for it in range(max_iter):
        ...
        Z = normalize(np.exp(loglik_amplify(_logLik_mat)))
        ...
        if it >= min_iter and _logLik_new - _logLik_old < epsilon_conv:
            if verbose:
                logging.debug("EM finished in %d iterations with logLik %.4e" %
                    (it, _logLik_new))
            break
        elif _logLik_new < _logLik_old:
            if verbose:
                logging.debug("Warning: likelihood decreases! it=%d, logLik_old=%.4e, logLik_new=%.4e." %
                    (it, _logLik_old, _logLik_new))
        else:
            if verbose:
                logging.debug("iter=%d, logLik_old=%.4e, logLik_new=%.4e." %
                    (it, _logLik_old, _logLik_new))
    ...
    dp_sum = DP.sum(axis = 0, keepdims = True)
